...
CampeonatoRouter.get('/list', IsAuth, async (req, res) => {
    ...
    try {
        const list = await client.campeonato.findMany({
            orderBy: { inicio: 'desc' },
            select: { idCampeonato: true, inicio: true, fim: true, inicioInscricoes: true, fimInscricoes: true },
        })
        res.status(200).json({ data: list, erros: null })
    } catch (err) {
        res.status(500).json({ data: null, erros: ['Falha ao listar campeonatos'] })
    }
})
...
CampeonatoRouter.post('/create', IsAuth, async (req, res) => {
    ...
    try {
        ...
        const fimDt = fim ? new Date(fim) : new Date(now.getTime() + 24*60*60*1000)
        const iniIns = inicioInscricoes ? new Date(inicioInscricoes) : now
        const fimIns = fimInscricoes ? new Date(fimInscricoes) : new Date(fimDt.getTime() - 24*60*60*1000)
        const created = await client.campeonato.create({ data: {
            inicio: inicioDt,
            fim: fimDt,
